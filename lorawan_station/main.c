...
void get_payload(char* message){
    ...
    int windir = get_rnd(0,360);
    int winint = get_rnd(0,100);
    int rain = get_rnd(0,50);
    sprintf(message,"{\"Temperature\": \"%u.%u\", \"Humidity\": \"%u.%u\", \"Wind Direction\": \"%d\", \"Wind Intensity\": \"%d\", \"Rain Height\": \"%d\"}",
    (temp / 10), (temp % 10), (hum / 10), (hum % 10), windir, winint, rain);
}
